    ...
    public function store(StoreBookingTransactionRequest $request)
    {
        try {
            ...
            if ($request->hasFile('proof')) {
                ...
                $validatedData['proof'] = $filePath; //proof/buktransfer.png
            }

            // Retrieve the service IDs from the request
            $serviceIds = $request->input('service_ids'); // No need to decade JSON anymore, as it's sent as an array
            ...
            // Use Carbon schedule_at to tomottow's date
            ...
            // Return the booking transaction data with details
            return new BookingTransactionApiResource($bookingTransaction->load('transactionDetails'));

        } catch (\Exception $se) {
            return response()->json(['message' => 'An error occurred', 'error' => $se->getMessage()], 500);
        }
    }
