...
export default function PaymentPage() {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (response.status === 200 || response.status === 201) {
                ...
                setSuccessMessage("Payment proof uploaded seccessfully!");
    ...
    return (
        <main className="relative min-h-screen mx-auto w-full max-w-[640px] bg-[#F4F5F7]">
            ...
            <section
                id="NavTop"
                className={`fixed left-0 right-0 z-30 transition-all duration-300 ${isScrolled ? 'top-[30px]' : 'top-[16px]'}`}
            >
                <div className="relative mx-auto max-w-[640px] px-5">
                    <div
                        id="ContainerNav"
                        className={`flex items-center justify-between py-[14px] transition-all duration-300 ${isScrolled ? 'bg-white rounded-[22px] px-[16px] shadow-[0px_12px_20px_0px_#0305041C]' : ''}`}
                    >
                        {/* Tombol Back */}
                        <Link to={`/booking`}>
                            <div
                                id="Back"
                                className={`flex h-[44px] w-[44px] shrink-0 items-center justify-center rounded-full bg-white ${isScrolled ? 'border border-shujia-graylight' : ''}`}
                            >
                                <img
                        ...
                        {/* Placeholder kanan */}
                        <div className="h-[44px] w-[44px]"></div>
                    </div>
                </div>
            </section>
