    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (response.status === 200 && response.data.data) {
                // Assumsing response contains booking details under 'response.data.data'
                ...
                <form onSubmit={handleSubmit}>
                    <section className="flex flex-col gap-4 rounded-3xl border border-shujia-graylight bg-white px-[14px] py-[14px]">
                        <label className="flex flex-col gap-2">
                            ...
                            {fomrErrors.find((error) => error.path.includes("booking_trx_id")) && (
                                <p>
                                    {
                                        fomrErrors.find((error) => error.path.includes("booking_trx_id"))
                                            ?.message
                        ...
                        <label className="flex flex-col gap-2">
                            ...
                            {fomrErrors.find((error) => error.path.includes("email")) && (
                                <p>
                                    {
                                        fomrErrors.find((error) => error.path.includes("email"))
                                            ?.message
                    ...
                    <div id="ResultBooking" className="space-y-[20px]">
                        ...
                        >
                            ...
                            {bookingDetails.is_paid ?
                                ...
                                :
                                <div id="BookingStatusJ incomplete" className="relative w-full pb-[42px]">
                                    <div className="flex">
                                        <div className="flex flex-col items-center">
                                            <div className="relative z-10 flex h-[25px] items-center">
                                                <div className="h-2 w-[60px] rounded-full bg-[#F4F5F7]" />
